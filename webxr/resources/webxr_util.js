...
function xr_session_promise_test(
  ...
  function runTest(t, sessionObjects) {
    let testSession;
  ...
  function createSessionObject(glContextType) {
    let canvas = document.createElement('canvas');
    document.body.appendChild(canvas);
    let gl = canvas.getContext(glContextType, {alpha: false, antialias: false, ...glcontextProperties});
    return { gl: gl };
  }

  xr_promise_test(
    name + ' - webgl',
    (t) => { runTest(t, createSessionObject('webgl')); },
    properties);
  xr_promise_test(
    name + ' - webgl2',
    (t) => { runTest(t, createSessionObject('webgl2')); },
    properties);
}
